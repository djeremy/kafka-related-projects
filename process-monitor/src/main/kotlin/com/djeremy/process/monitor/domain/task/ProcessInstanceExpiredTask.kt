...
open class DefaultProcessInstanceExpiredTask(
    ...
    override fun execute() {
        ...
        val notAdmittedInstances = processInstanceStateRepository.getNotAdmittedIdsBefore(beforeOffset)

        notAdmittedInstances.chunked(pageSize).forEach { projections ->
            admitAndAlert(projections, configurationsCache)
        }
    }

    @Transactional
    open fun admitAndAlert(
        data: List<ProcessInstanceStateProjection>,
        configurationCache: MutableMap<ProcessConfigurationId, ProcessConfiguration?>
    ) {
        val currentDate = now()
        ...
        processInstanceStateRepository.getBy(data.map { it.processInstance }).forEach {
    ...
    private fun MutableMap<ProcessConfigurationId, ProcessConfiguration?>.tryGetOrNull(
