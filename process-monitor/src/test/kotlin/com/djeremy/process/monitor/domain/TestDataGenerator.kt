...
fun `given step`(
    referenceIds: List<String>? = null,
    configurationId: String? = null,
    processInstanceId: ProcessInstanceId? = null,
    withProcessInstanceId: Boolean = false,
    isNewlyAssigned: Boolean = true,
    isLast: Boolean = false,
    receivedAt: LocalDateTime = now().truncatedTo(MILLIS)
) =
    Step(
...
fun `given single step configuration`(
    configurationId: String? = null, stepId: String? = null, isFirst: Boolean = false,
    isLast: Boolean = false
): StepConfigurationModel = SingleStepConfigurationModel(
...
fun `given multiple exclusive step configuration`(
    configurationId: String? = null,
    isLast: Boolean = false
): StepConfigurationModel = MultipleExclusiveStepConfigurationModel(
...
fun `given process configuration`(duration: Duration, configurationId: String? = null): ProcessConfiguration =
...
fun `given process instance state in NEW stage`(
    processInstanceId: ProcessInstanceId? = null,
    configurationId: String? = null,
    withSteps: List<Step> = emptyList()
    ...
    return createNew(step.first().getProcessInstance()!!, step.map { it.toView() })
        .let { it.copy(startedAt = it.startedAt.truncatedTo(MILLIS)) }
}

fun `given process instance state in FINISHED stage`(
    processInstanceId: ProcessInstanceId? = null,
    configurationId: String? = null,
    withSteps: List<Step> = emptyList()
): ProcessInstanceState {
...
fun `given process instance state in ADMITTED stage`(
    processInstanceId: ProcessInstanceId? = null,
    configurationId: String? = null,
    withSteps: List<Step> = emptyList()
): ProcessInstanceState {
