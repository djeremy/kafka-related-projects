...
internal class MappersKtTest : Spek({
    ...
    describe("Test resolve resolveStepModel") {
        ...
        it("should correctly map to Step when old processStep is received") {
            // given
            ...
            assertSoftly {
                ...
                it.assertThat(result.isLast).isEqualTo(true)

                it.assertThat(result.references.map { it.referenceId })
                        .containsOnly(*processStep.getReferenceIds().toTypedArray())
            }
        }
        it("should correctly map to Step when new processStep is received") {
            // given
            val processStep = `random ProcessStepV2`()
