...
@EmbeddedKafka(
    partitions = 1, topics = [COMMANDS_TOPIC, EVENTS1_TOPIC, EVENTS2_TOPIC, EVENTS3_TOPIC, PROCESS_STEP_EVENTS_TOPIC],
    brokerProperties = ["transaction.state.log.replication.factor=1", "transaction.state.log.min.isr=1"]
)
@ActiveProfiles(*["test", "single"])
internal class SingleConfigurationStateAggregationFlowTest {
    @Autowired
    ...
    fun `should join all events correctly`() {
        ...
        assertThat(result.first().stage)
            .isEqualTo(ProcessInstanceStage(isFinished = true, isAdmitted = false))
    }

    fun `wait for all steps to be processed`(
